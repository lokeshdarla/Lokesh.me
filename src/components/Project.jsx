...
const ProjectCard = ({ title, description, imageSrc, altText, link, previewLink }) => (
  <div className="flex items-center justify-center sm:flex-row lg:flex-row ">
    <p href="#" class="group relative block h-68 w-96">
  <span class="absolute inset-0 border-2 border-dashed border-black rounded-xl"></span>
    
  <div class="relative flex h-full transform items-center border-2 border-black rounded-xl bg-white transition-transform group-hover:-translate-x-2 group-hover:-translate-y-2">
    
    <div class="p-4 !pt-0 transition-opacity group-hover:absolute group-hover:opacity-0 sm:p-6 lg:p-8">
    <img src={imageSrc} alt={altText} className='mt-2'/>
    ...
    <div class="absolute p-4 opacity-0 transition-opacity group-hover:relative group-hover:opacity-100 sm:p-6 lg:p-8">
      ...
      <a href={link} className='px-5 py-2 mt-2  text-xs rounded-full border text-white bg-[#050708] hover:bg-[#050708]/90 focus:ring-4 focus:outline-none items-center gap-2 text-md inline-flex'>Github<span><RxExternalLink /></span></a>
    </div>
  </div>
</p>
